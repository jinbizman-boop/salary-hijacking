...
const assertPublicUrlTargetsMatch = ({
  proof,
  proofPath,
  expectedUrls,
  requireCheckedUrls = false,
}) => {
  const checkedUrls = isPlainObject(proof?.checkedUrls)
    ? proof.checkedUrls
    : {};
  if (requireCheckedUrls && Object.keys(checkedUrls).length === 0) {
    throw new Error(
      `${proofPath} public URL proof targets do not match release targets: checkedUrls is missing`,
    );
  }

  const mismatches = Object.entries(expectedUrls)
    .filter(([key, expectedValue]) => checkedUrls[key] !== expectedValue)
    .map(([key, expectedValue]) => `${key} must be ${expectedValue}`);

  if (mismatches.length > 0 && Object.keys(checkedUrls).length > 0) {
    throw new Error(
      `${proofPath} public URL proof targets do not match release targets: ${mismatches.join("; ")}`,
    );
  }
};
...
const readProof = (rootDir, proofPath, expectedUrls) => {
  const localProof = readJsonIfPresent(rootDir, proofPath);
  if (localProof) {
    return validateNoSecretProof(localProof, proofPath, expectedUrls, {
      requireCheckedUrls: true,
    });
  }
  ...
  const fallback =
    proofPath === DEFAULT_PROOF_PATH
      ...
      : null;
  if (fallback) {
    return validateNoSecretProof(fallback, DEFAULT_OUTPUT_PATH, expectedUrls);
  }
...
export const buildPublicUrlEvidence = ({
  ...
  const expectedUrls = readPublicUrlTargets(rootDir);
  const proof = readProof(rootDir, proofPath, expectedUrls);
