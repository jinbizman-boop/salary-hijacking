...
-- Re-applies the runtime upload metadata contract when a staging branch ledger
-- contains 0010 but the physical attachments table is still at the 0003 shape.
...
BEGIN
  IF to_regclass('public.attachments') IS NULL THEN
    RAISE EXCEPTION '0025_uploads_runtime_metadata_repair.sql requires public.attachments';
  END IF;
END;
$$;
...
ALTER TABLE public.attachments
  ...
  DROP CONSTRAINT IF EXISTS chk_attachments_upload_purpose,
  DROP CONSTRAINT IF EXISTS chk_attachments_upload_visibility,
  DROP CONSTRAINT IF EXISTS chk_attachments_scan_status,
  DROP CONSTRAINT IF EXISTS chk_attachments_file_name_length,
  DROP CONSTRAINT IF EXISTS chk_attachments_computed_checksum;
