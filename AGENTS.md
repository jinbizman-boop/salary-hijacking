...
- `corepack pnpm run test:root-scripts`: PASS, 131 tests
...
- `node --test scripts/release/check-release-readiness.test.mjs`: PASS, 41 tests; release readiness now blocks tracked external release evidence when raw secret values are embedded and blocks tracked mobile native evidence when `containsSecretValues` is not explicitly false or raw native release secret values are embedded
