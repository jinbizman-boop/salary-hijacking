...
- `node --test apps\mobile\scripts\eas-local-android-build.test.mjs`: PASS, 5 tests; the local Android E2E build wrapper checks EAS `e2e` APK profile, workspace-local EAS CLI, Expo authentication via `EXPO_TOKEN` or `eas whoami`, Android SDK tools, Java/JBR availability, the exact Detox APK output path, non-interactive local EAS flags, and Windows `.CMD` shell execution
