...
- `node --test scripts/release/check-release-readiness.test.mjs`: PASS, 47 tests; release readiness now blocks tracked secret evidence when raw values, raw secret-like strings in notes, unknown secret names, or unapproved store labels are embedded, blocks tracked external release evidence when raw secret values are embedded, blocks tracked Cloudflare runtime evidence when unrelated Worker names are embedded or when its expected Worker names drift from `release/release-targets.json`, and blocks tracked mobile native evidence when `containsSecretValues` is not explicitly false, raw native release secret values are embedded, or unsafe native release privacy flags are set
