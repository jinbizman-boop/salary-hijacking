...
- `corepack pnpm run test:root-scripts`: PASS, 187 tests; root script tests now run through `scripts/dev/run-node-tests-with-clean-temp.mjs` so Node test fixtures and compile cache are isolated under a cleanup directory instead of accumulating in the default C-drive temp path, and `scripts/dev/clean-generated-junk.mjs` is covered for safe generated-artifact cleanup
