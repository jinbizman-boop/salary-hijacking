...
- `corepack pnpm run test:root-scripts`: PASS, 134 tests
...
- `node --test scripts/release/check-release-readiness.test.mjs`: PASS, 44 tests; release readiness now blocks tracked external release evidence when raw secret values are embedded, blocks tracked Cloudflare runtime evidence when unrelated Worker names are embedded or when its expected Worker names drift from `release/release-targets.json`, and blocks tracked mobile native evidence when `containsSecretValues` is not explicitly false, raw native release secret values are embedded, or unsafe native release privacy flags are set
