...
- `corepack pnpm run check:external-integrations`: PASS, 67 required files checked, including public app/legal custom-domain targets, no-secret public URL evidence/proof automation, no-secret database command proof automation, no-value runtime secret proof automation, and no-secret Cloudflare observation proof automation; this local shell warns `wrangler`, `gh`, `neon`, and `neonctl` are not on PATH for that script
- `corepack pnpm run test:root-scripts`: PASS, 100 tests
