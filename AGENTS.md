...
- `corepack pnpm run test:root-scripts`: PASS, 190 tests; root script tests now run through `scripts/dev/run-node-tests-with-clean-temp.mjs` so Node test fixtures and compile cache are isolated under a cleanup directory instead of accumulating in the default C-drive temp path, `scripts/dev/clean-generated-junk.mjs` is covered for safe generated-artifact cleanup, and `scripts/dev/report-workspace-disk-usage.mjs` provides a no-delete disk report for protected dependencies versus removable caches
