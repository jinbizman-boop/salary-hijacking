...
- `corepack pnpm run check:external-integrations`: PASS, 78 required files checked, including public app/legal custom-domain targets, release workflow mojibake guard, GitHub Actions runtime proof artifact guard, no-secret public URL proof artifact guard, no-secret public URL evidence/proof automation, no-secret database command proof automation, no-value runtime secret proof automation, no-secret Cloudflare observation proof automation, no-secret mobile native observation proof automation, no-secret dependency security audit evidence automation, and tracked no-secret local proof example templates; this local shell warns `gh`, `neon`, and `neonctl` are not on PATH for that script
- `corepack pnpm run test:root-scripts`: PASS, 173 tests
