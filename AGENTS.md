...
- `corepack pnpm run test:root-scripts`: PASS, 130 tests
...
- `node --test scripts/release/check-release-readiness.test.mjs`: PASS, 40 tests; release readiness now blocks tracked mobile native evidence when `containsSecretValues` is not explicitly false or raw native release secret values are embedded
