...
- `corepack pnpm run test:root-scripts`: PASS, 196 tests; root script tests now run through `scripts/dev/run-node-tests-with-clean-temp.mjs` so Node test fixtures and compile cache are isolated under a cleanup directory instead of accumulating in the default C-drive temp path, Windows cleanup retries are covered for transient `EBUSY` locks, `scripts/dev/clean-generated-junk.mjs` is covered for safe generated-artifact cleanup, `scripts/dev/report-workspace-disk-usage.mjs` provides a no-delete disk report for protected dependencies versus removable caches, and `apps/mobile/scripts/eas-local-android-build.test.mjs` covers the guarded local Android E2E EAS build wrapper
- `node --test apps\mobile\scripts\eas-local-android-build.test.mjs`: PASS, 4 tests; the local Android E2E build wrapper checks EAS `e2e` APK profile, workspace-local EAS CLI, Android SDK tools, Java/JBR availability, the exact Detox APK output path, non-interactive local EAS flags, and Windows `.CMD` shell execution
...
- `corepack pnpm --filter @salary-hijacking/mobile run build:e2e:android:preflight`: PASS, detected Android Studio JBR at `C:\Program Files\Android\Android Studio\jbr` and Android SDK root at `C:\Users\Telos_PC_17\AppData\Local\Android\Sdk`
