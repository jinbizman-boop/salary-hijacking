...
| GAP-008 | P0       | External approval | Production AAB/Play release intent is approved by the user for Android-only launch, and the release/control-plane HEAD is pushed to `origin/main`. Final Production AAB, EAS authentication, same-RC Galaxy runtime proof, and Play track execution remain separate release gates and are not implied by this external-approval row.                                                                                                                       | user launch approval, `origin/main` lineage, separate release-gate evidence for AAB/Galaxy/Play                 | PASS    |
