...
- `corepack pnpm run check:external-integrations`: PASS, 78 required files checked; release target policy, source automation trackability, API/Admin deploy workflow dispatch-only guarding, deploy workflow mojibake guarding, no-secret release proof paths, GitHub Environments runtime secret proof artifact guarding, Cloudflare runtime proof artifact guarding, database command proof artifact guarding, release readiness workflow gating, and Android blocker wording guards are included; local shell warns that `gh`, `neon`, and `neonctl` are not on PATH
- `node --test scripts/release/check-release-readiness.test.mjs`: PASS, 54 tests
- `corepack pnpm run test:root-scripts`: PASS, 200 tests; root script tests now run through `scripts/dev/run-node-tests-with-clean-temp.mjs` so temporary fixtures and Node compile cache are isolated under a cleanup directory instead of accumulating in the default C-drive temp path, with Windows-safe cleanup retries for transient `EBUSY` locks. `corepack pnpm run clean:junk` is now available for post-verification cleanup of regenerated caches, native EAS/Android/iOS build caches, local proof files, and Salary Hijacking temp fixtures without removing `node_modules`, local secret files, tracked release evidence, or explicit E2E APK paths. `corepack pnpm run disk:report -- --top 20` is available as a no-delete report for top-level disk usage, protected dependencies, and removable generated cache paths. `apps/mobile/scripts/eas-local-android-build.test.mjs` covers the guarded local Android E2E EAS build wrapper, including Expo authentication gating before local Android E2E build attempts. API/Admin Cloudflare deploy jobs are now workflow-dispatch-only so push/PR runs verify without trying to deploy before runtime secrets and release evidence are ready.
- `corepack pnpm run check:release-readiness -- --soft`: PASS as a reporting command, with release status `BLOCKED`; GitHub target evidence, local `origin`, authenticated push evidence, workspace-local EAS CLI evidence, Cloudflare Wrangler availability, and Android SDK tool lookup are aligned with the Salary Hijacking target. GitHub CLI and Neon CLI absence are WARN when connector evidence proves account access. Android `adb`, `emulator`, and Java/JBR are detected, but the local EAS Android build preflight now fails fast until Expo authentication is available through `eas login` or trusted `EXPO_TOKEN`; native E2E remains blocked until that authentication produces a local Detox E2E APK or equivalent no-secret native/device-farm proof.
...
- `corepack pnpm --filter @salary-hijacking/mobile run build:e2e:android:preflight`: FAIL fast because Expo account authentication is unavailable; Android Studio JBR and the local Android SDK root are detected
- `corepack pnpm --filter @salary-hijacking/mobile run build:e2e:android:local`: BLOCKED by the same Expo account authentication requirement through `eas login` or a trusted `EXPO_TOKEN`; no Android E2E APK was produced
...
- "native mobile E2E blocked by missing Detox APK or equivalent no-secret native/device-farm proof"
