...
- `corepack pnpm run test:root-scripts`: PASS, 199 tests; root script tests now run through `scripts/dev/run-node-tests-with-clean-temp.mjs` so temporary fixtures and Node compile cache are isolated under a cleanup directory instead of accumulating in the default C-drive temp path, with Windows-safe cleanup retries for transient `EBUSY` locks. `corepack pnpm run clean:junk` is now available for post-verification cleanup of regenerated caches, native EAS/Android/iOS build caches, local proof files, and Salary Hijacking temp fixtures without removing `node_modules`, local secret files, tracked release evidence, or explicit E2E APK paths. `corepack pnpm run disk:report -- --top 20` is available as a no-delete report for top-level disk usage, protected dependencies, and removable generated cache paths. `apps/mobile/scripts/eas-local-android-build.test.mjs` covers the guarded local Android E2E EAS build wrapper. API/Admin Cloudflare deploy jobs are now workflow-dispatch-only so push/PR runs verify without trying to deploy before runtime secrets and release evidence are ready.
- `corepack pnpm run check:release-readiness -- --soft`: PASS as a reporting command, with release status `BLOCKED`; GitHub target evidence, local `origin`, authenticated push evidence, workspace-local EAS CLI evidence, Cloudflare Wrangler availability, and Android SDK tool lookup are aligned with the Salary Hijacking target. GitHub CLI and Neon CLI absence are WARN when connector evidence proves account access. Android `adb`, `emulator`, Java/JBR, and the EAS local build preflight are detected, but native E2E remains blocked until Expo authentication produces a local Detox E2E APK or equivalent no-secret native/device-farm proof.
...
- `corepack pnpm --filter @salary-hijacking/mobile run build:e2e:android:preflight`: PASS, detecting Android Studio JBR and the local Android SDK root on this PC
- `corepack pnpm --filter @salary-hijacking/mobile run build:e2e:android:local`: FAIL after reaching EAS CLI because Expo account authentication is required through `eas login` or a trusted `EXPO_TOKEN`; no Android E2E APK was produced
