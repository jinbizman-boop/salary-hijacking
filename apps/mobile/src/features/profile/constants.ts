export const PROFILE_PATH = "/api/v1/users/me/profile";
export const PROFILE_CONSENTS_PATH = "/api/v1/users/consents";
export const PROFILE_MY_PAGE_SUMMARY_PATH = "/api/v1/users/me/my-page-summary";
export const PROFILE_PRIVACY_EXPORT_PATH = "/api/v1/users/me/privacy-export";
export const PROFILE_WITHDRAWAL_REQUEST_PATH =
  "/api/v1/users/me/withdrawal-request";
export const PROFILE_SUPPORT_TICKETS_PATH = "/api/v1/users/me/support-tickets";
export const PROFILE_SAFE_ERROR_MESSAGE =
  "프로필 정보를 안전하게 불러오지 못했어요. 잠시 후 다시 시도해 주세요.";
