...
describe("level feature components", () => {
  ...
  it("renders reading, news, english, and workout cards with policy guards", () => {
    ...
    expect(screen.getByText("private LV UP record")).toBeTruthy();
  ...
  it("renders XP reward toast with server authority and idempotency guards", () => {
    const screen = render(
      <XpRewardToast earnedXp={30} rewardSource="READING_COMPLETE" />,
    );

    expect(screen.getByLabelText("READING_COMPLETE 30 XP")).toBeTruthy();
    expect(screen.getByText("READING_COMPLETE +30 XP")).toBeTruthy();
    expect(screen.getByText("serverAuthority=true")).toBeTruthy();
    expect(screen.getByText("idempotencyRequired=true")).toBeTruthy();
  });
});
