...
export type FixedExpenseFormValue = Readonly<{
  day: string;
  category: string;
  content: string;
  amount: string;
...
export type FixedExpenseFormScreenProps = Readonly<{
  initialValue?: Partial<FixedExpenseFormValue>;
  onCancel: () => void;
  onSave: (value: FixedExpenseFormValue) => void;
}>;
...
export function FixedExpenseFormScreen({
  ...
  onCancel,
  onSave,
}: FixedExpenseFormScreenProps) {
  const insets = useOptionalSafeAreaInsets();
  const [value, setValue] = useState<FixedExpenseFormValue>({
    amount: initialValue?.amount ?? "",
    category: initialValue?.category ?? "구독료 납부",
    content: initialValue?.content ?? "",
    day: initialValue?.day ?? "10",
  });

  const update = (key: keyof FixedExpenseFormValue, next: string) =>
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.title}>월별 고정 지출 계획/설정</Text>
        <Text style={styles.description}>
          매월 반복되는 지출일, 카테고리, 내용, 금액을 설정합니다.
        </Text>
        <TextInput
          accessibilityLabel="지출일"
